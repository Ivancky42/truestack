...
2. In the same change, update the same key in `messages/ms/<namespace>.json` and `messages/zh/<namespace>.json`. Keep the key path and array length identical.
3. If you add a key, add it in all three files. If you delete a key, delete it in all three.
...
1. Create `messages/{en,ms,zh}/<file>.json` with the same keys. English `_status: source`; ms/zh `_status: ai-draft`.
2. Import English in `lib/i18n/messages.tsx` (`EnMessages` intersection **and** `englishMessages` merge).
3. Import the same file in `messages/ms/index.ts` and `messages/zh/index.ts` and pass it to `namespaces(...)`.
...
- No hype words. zh titles: primary keyword in the first 10 code points, ≤ 30; descriptions 70–90. en/ms titles ≤ 60; descriptions 140–160.
...
`pnpm i18n:check`, then `pnpm lint` and `pnpm build`. Spot-check `/path`, `/ms/path`, `/zh/path` (title, canonical, hreflang, body not leftover English).
