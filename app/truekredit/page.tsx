...
const HERO_FEATURES = [
	...
	{
		label: "Arrears & Default",
		icon: AlertTriangle,
		color: "text-amber-600",
		bg: "bg-amber-50",
	},
	{
		label: "Receipt & Letters",
		icon: Receipt,
		color: "text-emerald-600",
		bg: "bg-emerald-50",
	},
		...
		>
			...
			>
				{HERO_FEATURES.map((_, i) => {
					...
					const radiusY = 200;
					const nx =
						Math.round((260 + Math.cos(angle) * radiusX) * 100) /
						100;
					const ny =
						Math.round((260 + Math.sin(angle) * radiusY) * 100) /
						100;
					return (
						<motion.line
							key={i}
			...
			{HERO_FEATURES.map((feature, i) => {
				...
				const px =
					...
					offset.x *
						parallax *
						(0.6 + 0.4 * Math.abs(Math.cos(angle)));
				const py =
					baseY +
					offset.y *
						parallax *
						(0.6 + 0.4 * Math.abs(Math.sin(angle)));
